#!/usr/bin/env python3
"""
Ultra-Extended Training with Resume Capability: Massive-scale training for Military RL Agent.

This script can resume from checkpoints and run extremely long training sessions
to achieve 10-20% state coverage of the 645,120 possible military states.
"""
...
# Move heavy imports inside worker function to avoid multiprocessing circular import issues
...
def create_ultra_diverse_tribe(tribal_manager, world, episode_num, tribe_idx, seed_offset, diversity_level=1.0):
    ...
    # Ultra-diverse tribe characteristics
    tribe_types = [
...
def run_ultra_episode(args):
    ...
    # Import here to avoid multiprocessing circular import issues
    from world.engine import WorldEngine
    from tribes.tribal_manager import TribalManager
    from rl_military_agent import MilitaryRLAgent
    from rl_military_interface import (
    ...
    try:
        ...
        # Agent with varying exploration
        epsilon = max(0.05, 0.3 - episode_num * 0.00001)
        ...
        # Run for 200 ticks with decisions every 6 ticks
...
def save_checkpoint(master_agent, episode_num, total_states, start_time, checkpoint_dir="artifacts/checkpoints"):
    ...
    # Save Q-table
    checkpoint_path = f"{checkpoint_dir}/military_qtable_checkpoint_ep{episode_num}.json"
...
def run_ultra_training_resume(num_episodes=20000, num_workers=8, checkpoint_interval=1000):
    """Run ultra-extended training with resume capability."""
    from rl_military_agent import MilitaryRLAgent

    print("Starting Ultra-Extended Military RL Training (with Resume)")
    ...
    # Final save
    final_path = "artifacts/models/military_qtable_ultra_extended_final.json"
...
if __name__ == "__main__":
    # Run ultra-extended training with resume capability
    run_ultra_training_resume(num_episodes=20000, num_workers=8, checkpoint_interval=1000)